...
// Standalone marked library detection (from marked.min.js)
...
const BASE_PATH = pathParts.join('/').replace(/\/+$/, '');

function apiPath(endpoint) {
  let [p, query] = (endpoint.startsWith("/") ? endpoint : "/" + endpoint).split("?");

  if (p === "/api/auth/login" || p === "/api/login" || p === "/api/admin/login") {
    p = "/api/auth.php";
    query = (query ? query + "&" : "") + "action=login";
  } else if (p === "/api/auth/logout" || p === "/api/logout" || p === "/api/admin/logout") {
    p = "/api/auth.php";
    query = (query ? query + "&" : "") + "action=logout";
  } else if (p === "/api/auth/me" || p === "/api/auth/check-session" || p === "/api/admin/check-session" || p === "/api/auth/status") {
    p = "/api/auth.php";
    query = (query ? query + "&" : "") + "action=me";
  } else if (p === "/api/auth/change-password") {
    p = "/api/auth.php";
    query = (query ? query + "&" : "") + "action=change-password";
  } else if (p === "/api/admin/settings" || p === "/api/settings") {
    p = "/api/settings.php";
  } else if (p === "/api/admin/test-wp-connection") {
    p = "/api/settings.php";
    query = (query ? query + "&" : "") + "action=test-wp-connection";
  } else if (p === "/api/articles/sync-wp" || p === "/api/articles/sync") {
    p = "/api/sync-wp.php";
  } else if (p === "/api/articles/schedule-publish" || p === "/api/publish-wordpress") {
    p = "/api/publish-wordpress.php";
  } else if (p === "/api/articles/import-csv") {
    p = "/api/articles.php";
    query = (query ? query + "&" : "") + "action=import-csv";
  } else if (p === "/api/articles/export-csv") {
    p = "/api/articles.php";
    query = (query ? query + "&" : "") + "action=export-csv";
  } else if (p.startsWith("/api/articles/")) {
    const artId = p.replace("/api/articles/", "");
    p = "/api/articles.php";
    query = (query ? query + "&" : "") + "id=" + artId;
  } else if (p === "/api/articles") {
    p = "/api/articles.php";
  } else if (p.startsWith("/api/queue/")) {
    const qId = p.replace("/api/queue/", "");
    p = "/api/queue.php";
    query = (query ? query + "&" : "") + "id=" + qId;
  } else if (p === "/api/queue") {
    p = "/api/queue.php";
  } else if (p === "/api/upload" || p === "/api/upload-image") {
    p = "/api/upload.php";
  } else if (p === "/api/generate") {
    p = "/api/generate.php";
  } else if (p === "/api/update-section") {
    p = "/api/update-section.php";
  } else if (p === "/api/insert-link") {
    p = "/api/insert-link.php";
  } else if (p === "/api/generate-image-meta") {
    p = "/api/generate-image-meta.php";
  }

  const finalUrl = query ? `${p}?${query}` : p;
  return `${BASE_PATH}${finalUrl}`;
}
...
function initDom() {
  ...
  dom.closeProfileModal = $("#closeProfileModal");
  dom.cancelProfileBtn = $("#cancelProfileBtn");
  dom.saveProfileBtn = $("#saveProfileBtn");
  dom.btnOpenCredentialsFromSettings = $("#btnOpenCredentialsFromSettings");
  dom.profileUsernameInput = $("#profileUsernameInput");
  dom.profileNameInput = $("#profileNameInput");
  dom.profileOldPasswordInput = $("#profileOldPasswordInput");
  dom.profileNewPasswordInput = $("#profileNewPasswordInput");
  dom.profileConfirmPasswordInput = $("#profileConfirmPasswordInput");
  dom.profileAlert = $("#profileAlert");
...
function openProfileModal() {
  ...
  if (dom.profileAlert) {
    dom.profileAlert.style.display = "none";
    dom.profileAlert.textContent = "";
  }

  if (dom.profileUsernameInput) dom.profileUsernameInput.value = state.user?.username || "admin";
  if (dom.profileNameInput) dom.profileNameInput.value = state.user?.name || "Administrator";
  if (dom.profileOldPasswordInput) dom.profileOldPasswordInput.value = "";
  if (dom.profileNewPasswordInput) dom.profileNewPasswordInput.value = "";
  if (dom.profileConfirmPasswordInput) dom.profileConfirmPasswordInput.value = "";

  if (dom.profileModal) dom.profileModal.classList.add("active");
  if (dom.profileOldPasswordInput) {
    setTimeout(() => dom.profileOldPasswordInput.focus(), 150);
  }
}
...
async function handleSaveProfile() {
  const username = dom.profileUsernameInput ? dom.profileUsernameInput.value.trim() : "";
  const name = dom.profileNameInput ? dom.profileNameInput.value.trim() : "";
  const oldPassword = dom.profileOldPasswordInput ? dom.profileOldPasswordInput.value.trim() : "";
  const newPassword = dom.profileNewPasswordInput ? dom.profileNewPasswordInput.value.trim() : "";
  const confirmPassword = dom.profileConfirmPasswordInput ? dom.profileConfirmPasswordInput.value.trim() : "";

  if (!oldPassword) {
    if (dom.profileAlert) {
      dom.profileAlert.className = "auth-alert error";
      dom.profileAlert.textContent = "Password saat ini wajib diisi untuk verifikasi keamanan.";
      dom.profileAlert.style.display = "block";
    }
    if (dom.profileOldPasswordInput) dom.profileOldPasswordInput.focus();
    return;
  }
  ...
  if (newPassword && newPassword !== confirmPassword) {
    if (dom.profileAlert) {
      dom.profileAlert.className = "auth-alert error";
      dom.profileAlert.textContent = "Konfirmasi password baru tidak cocok.";
      dom.profileAlert.style.display = "block";
    }
    if (dom.profileConfirmPasswordInput) dom.profileConfirmPasswordInput.focus();
    return;
  }
  ...
  if (newPassword && newPassword.length < 4) {
    if (dom.profileAlert) {
      dom.profileAlert.className = "auth-alert error";
      dom.profileAlert.textContent = "Password baru minimal 4 karakter.";
      dom.profileAlert.style.display = "block";
    }
    if (dom.profileNewPasswordInput) dom.profileNewPasswordInput.focus();
    return;
  }

  if (dom.saveProfileBtn) {
    dom.saveProfileBtn.disabled = true;
    dom.saveProfileBtn.textContent = "Menyimpan...";
  }

  try {
    const token = state.token || localStorage.getItem("af-auth-token") || "";
    const res = await fetch(apiPath("/api/auth.php?action=change-password"), {
      method: "POST",
      headers: {
    ...
    if (dom.profileAlert) {
      dom.profileAlert.className = "auth-alert success";
      dom.profileAlert.textContent = data.message || "Kredensial berhasil diperbarui!";
      dom.profileAlert.style.display = "block";
    }

    // Update state and UI header
    if (data.user) {
      state.user = { ...state.user, ...data.user };
      if (dom.userHeaderName) dom.userHeaderName.textContent = data.user.username;
    }

    showToast(data.message || "Kredensial berhasil diperbarui!", "success");
    setTimeout(() => {
      closeProfileModal();
    }, 1200);
    ...
    if (dom.profileAlert) {
      dom.profileAlert.className = "auth-alert error";
      dom.profileAlert.textContent = err.message;
      dom.profileAlert.style.display = "block";
    }
  } finally {
    if (dom.saveProfileBtn) {
      dom.saveProfileBtn.disabled = false;
      dom.saveProfileBtn.textContent = "Save Credentials";
    }
  }
}
...
function init() {
  ...
  // Profile & Credentials Modal Listeners
  if (dom.userProfileBadge) dom.userProfileBadge.addEventListener("click", openProfileModal);
  if (dom.btnOpenCredentialsFromSettings) dom.btnOpenCredentialsFromSettings.addEventListener("click", openProfileModal);
  if (dom.closeProfileModal) dom.closeProfileModal.addEventListener("click", closeProfileModal);
  if (dom.cancelProfileBtn) dom.cancelProfileBtn.addEventListener("click", closeProfileModal);
  if (dom.saveProfileBtn) dom.saveProfileBtn.addEventListener("click", handleSaveProfile);
  if (dom.profileModal) {
...
function renderQueue() {
  ...
  state.queue.forEach((item) => {
    ...
    // Action buttons for queue list items: Regenerate, Publish, Schedule, Remove (Admin only), View Post Link
    const qIdStr = escapeHtml(String(item.id));
    const regenBtn = item.status !== "generating" ? `<button class="btn btn-xs btn-secondary" onclick="event.stopPropagation(); window.__regenerateQueueItem('${qIdStr}')" title="Regenerate this article">🔄 Regenerate</button>` : "";
...
async function generateArticle(item) {
  ...
  try {
    ...
    if (item.status === "generating") {
      item.status = "error";
      item.error = "Connection closed before completion.";
      showToast(`Generation stopped unexpectedly: "${item.title}"`, "error");
    }
  } catch (err) {
    item.status = "error";
...
function handleSSEEvent(item, data) {
  switch (data.type) {
    ...
    case "progress":
      ...
      break;
    case "outline":
      if (data.sections) item.outline = data.sections;
      if (data.percent) item.progress = data.percent;
      break;
    case "section_done":
      if (data.wordCount) item.wordCount = data.wordCount;
      if (data.percent) item.progress = data.percent;
      break;
    case "delta":
...
function renderPreview() {
  ...
  if (dom.previewRendered) {
    dom.previewRendered.innerHTML = featuredBannerHtml + renderedHtml;
    // Auto-apply modern TL;DR callout styling to any blockquote containing TL;DR or Key Takeaways
    dom.previewRendered.querySelectorAll('blockquote').forEach(bq => {
      if (/tl;?dr|key takeaways/i.test(bq.textContent)) {
        bq.classList.add('tldr-box');
      }
    });
  }
...
function renderArticles() {
  ...
  filtered.forEach(item => {
    ...
    if (queueItem) {
      ...
      } else if (queueItem.status === "telah_dibuat") {
        badgeClass = "queue-badge-telah";
        badgeText = "✅ Telah Dibuat (Di Antrean)";
      } else if (queueItem.status === "dijadwalkan") {
        badgeClass = "queue-badge-dijadwalkan";
        badgeText = "📅 Dijadwalkan (Di Antrean)";
      } else if (queueItem.status === "complete" || queueItem.status === "completed") {
...
async function saveScheduleModal() {
  ...
  try {
    const res = await authFetch(apiPath("/api/articles/schedule-publish"), {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        articleId: targetItem.managerId || targetItem.id,
